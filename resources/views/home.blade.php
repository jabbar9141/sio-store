                    ...
                    @foreach ($categories as $category)
                        <div class="item">
                            <a href="#" class="banner-img">
                                <img src="{{ asset('uploads/images/category/' . $category->category_image) }}" alt="">
                            </a>
                        </div>
                    @endforeach
                @endif
                                ...
                                <a href="{{ route('store.showProduct', $r->product_slug) }}">
                                    <div class="top-deal-card">
                                        ...
                                        <div class="top-deal-content mt-3">
                                            ...
                                            <h5>{{ session()->get('session_symbol') ?? '€' }}
                                                {{ number_format(App\MyHelpers::getPrice($r->product_price)) }}
                                            </h5>
                ...
                <!--<h1>HArsht</h1>-->
                 @if ($categories)
                        ...
                        <div class="product-grid px-3">
                            <?php
                               $products = DB::table('product')->where('category_id', $category->category_id)->where('admin_approved', 1)->where('product_quantity', '>', 0)->where('product_status', 1)->orderBy('product_id', 'DESC')->limit(4)->get();
                                if ($products->count() > 0) {
                                    foreach ($products as $product) {
                                        // dd($product);
                                ?>
                               <a href="{{ route('store.showProduct', $product->product_slug) }}" class="product-card">
